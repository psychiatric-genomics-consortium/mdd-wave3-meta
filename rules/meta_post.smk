...
rule postimp_ma:
    ...
    shell: """zcat {input} | awk '{{if(NR == 1) {{print $0}} else {{print $0 | "sort -k1,1n -k3,3n"}}}}' | awk '{{if(NR == 1) {{print "SNP", "A1", "A2", "freq", "beta", "se", "p", "N"}} else {{print $2, $4, $5, $7, log($9), $10, $11, 2*$19}}}}' | awk 'NR == 1 || $4 != 0' > {output}"""
...
rule postimp_dentist_chr:
    input: expand("results/meta/dentist/{cohort}_{ancestries}_v{version}/pgc_mdd_{cohort}_{ancestries}_hg19_v{version}.chr{chr}.DENTIST.txt", cohort='full', ancestries='eur', version=analysis_version, chr=range(1, 22))
