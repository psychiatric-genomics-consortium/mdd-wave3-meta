...
# 	input: expand("results/ldsc/pairs/{cohort1},{cohort2}.log", cohort1=ldsc_munged_cohorts, cohort2=ldsc_munged_cohorts)
...
# using the sorted combinations of sumstats
rule meta_ldsc_sumstats_pairs_table:
	input: ["results/sumstats/rg_pairs/%s,%s.log" % cohorts for cohorts in list(itertools.combinations(sorted(meta_ldsc_munged_cohorts), r=2))]
...
## $ sbatch -t 8:00:00 --wrap "snakemake -j4 --use-conda --cluster 'sbatch -t 60 -n 16' docs/tables/meta_qc_align.txt --groups text2daner=group0 hg19=group0 daner=group0 align=group0 filter=group0 meta_align_qc=group1 --group-components group0=4 group1=1"
