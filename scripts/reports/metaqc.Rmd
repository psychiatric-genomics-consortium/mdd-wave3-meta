...
Marker names were aligned to the meta-analysis reference panel and effect sizes were checked.

```{r meta_qc_align}
meta_qc_align <- read_tsv(snakemake@input$meta_qc_align)
```

Mean odds-ratios with standard error
...
ggplot(meta_qc_align %>%
       unite('cohort',
	    ...
	    aes(x=reorder(cohort, mean_SE),
		    y=mean_OR,
			ymax=exp(log(mean_OR)+mean_SE),
		    ymin=exp(log(mean_OR)-mean_SE))) +
geom_linerange() +
geom_point() +
