...
for(i in 1:22){
  if(i != 6){
    pwas_rosmap<-rbind(pwas_rosmap, fread(paste0('results/pwas/PGC_MDD3_pwas_rosmap_chr',i)))
  } else {
    pwas_rosmap<-rbind(pwas_rosmap, fread(paste0('results/pwas/PGC_MDD3_pwas_rosmap_chr',i)))
    pwas_rosmap<-rbind(pwas_rosmap, fread(paste0('results/pwas/PGC_MDD3_pwas_rosmap_chr',i,'.MHC')))
  }
}
...
pwas_banner<-pwas_banner[order(pwas_banner$TWAS.P),]
pwas_banner<-pwas_banner[!duplicated(pwas_banner$ID),]
...
pwas_rosmap_sig_banner_smr_rosmap$smr_replicated<-pwas_rosmap_sig_banner_smr_rosmap$p_SMR.FDR < 0.05 & sign(pwas_rosmap_sig_banner_smr_rosmap$rosmap.TWAS.Z) == sign(pwas_rosmap_sig_banner_smr_rosmap$b_SMR) 
...
# Show results that were significant in ROSMAP, Banner PWAS, and colocalised in ROSMAP PWAS, ROSMAP SMR
sig_coloc_all_table<-pwas_rosmap_sig_banner_smr_rosmap[pwas_rosmap_sig_banner$banner_replicated == T & pwas_rosmap_sig_banner_smr_rosmap$rosmap.causal == T & pwas_rosmap_sig_banner_smr_rosmap$smr.causal == T,]
...
write.csv(pwas_rosmap_sig_banner_smr_rosmap, 'results/pwas/rosmap_banner_pwas_smr_results.csv', row.names=F, quote=T)

```

<details><summary>Significant in discovery and replication PWAS, and colocalised using ROSMAP COLOC and ROSMAP HEIDI</summary>
