...
```{r nearest, eval=T, echo=T, cache=TRUE}
...
```{r fine, eval=T, echo=T, cache=TRUE}
...
```{r fastbat, eval=T, echo=T, cache=TRUE}
##########
# FastBAT
##########
...
```{r hmagma, eval=T, echo=T, cache=TRUE}
...
```{r twas, eval=T, echo=T, cache=TRUE}
##########
# TWAS
##########
...
```{r coloc, eval=T, echo=T, cache=TRUE}
...
```{r focus, eval=T, echo=T, cache=TRUE}
...
```{r expression, eval=T, echo=T, cache=TRUE}
...
```{r smr, eval=T, echo=T, cache=TRUE}
...
```{r heidi, eval=T, echo=T, cache=TRUE}
...
```{r pwas, eval=T, echo=T, cache=TRUE}
##########
# PWAS
##########
...
```{r psyops, eval=T, echo=T, cache=TRUE}
...
gene_overlap[['fastBAT']]<-fastbat$Gene[fastbat$Pvalue < 5e-8]
...
for(i in 1:nrow(high_conf_tab)){
  if(high_conf_tab$ENSGID[i] %in% fastbat$Gene){
    if(high_conf_tab$ENSGID[i] %in% fastbat$Gene[fastbat$Pvalue < 5e-8]){
...
genes_methods_conf <-
dplyr::arrange(
  dplyr::filter(genes_methods_cpid, Nearest_gene | Fine_mapping | Expression | Protein | fastBAT | PsyOPS),
