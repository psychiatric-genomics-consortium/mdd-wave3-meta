...
```{r library,echo=F,warning=F,error=F}
library(dplyr)
library(tidyverse)
library(data.table)
library(readr)
...
render('scripts/phewas/SUMMARY/summary.mr_phewas.Rmd',output_dir='docs/',output_file='summary.mr_phewas.md')
```
...
```{r load refs and categories, warning=F, echo=F}
...
```{r load ukb mr phewas results, warning=F,echo=F,error=F}
...
mr.ukb = mr.ukb %>% 
  ...
  mutate(b=ifelse(exposure %in% ls.neale.gwas,-b,b)) %>% 
  mutate(b=ifelse(outcome %in% ls.neale.gwas,-b,b))
```
...
```{r load mr-base mr phewas results, warning=F,echo=F,error=F}
...
figs.mr_base.mdd_to_pheno=beta_fig(tmp.dat.fig=dat.fig,is.allres=T)

figs.mr_base.mdd_to_pheno
```
