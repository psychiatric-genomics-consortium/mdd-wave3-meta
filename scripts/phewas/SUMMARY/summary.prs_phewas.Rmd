...
res.targetpT = list.files(g.path,pattern='^phewas_out',full.names = T,include.dirs = T) %>% 
  ...
  mutate(p.value = as.numeric(p.value),
         beta=as.numeric(beta),std=as.numeric(std),t.value=as.numeric(t.value)) %>% 
...
pt.prs=tmp.findpt$factor %>% head(n=1) %>% gsub('MDD3_Pt_','',.)
target.pT = pt.prs %>% 
  gsub('\\.','\\\\.',.) %>% 
  paste0('Pt_',.)
...
fig.MDD3=p_plot(TargetResult=plot.dat,color_theme_usr='Shen',
           ...
           plot_title=paste0('MDD3 PRS (pT=',pt.prs,')'),y_lim=310,
...
fig.Howard=p_plot(TargetResult=plot.dat,color_theme_usr='Shen',
                shape_sig=T,sig_nominal=F,
           plot_title=paste0('Howard PRS (pT=',pt.prs,')'),y_lim=310,
           labels_annot=ls.label,add_category_name=T)
...
-   CI for MDD3 PRS and Howard et al PRS do not overlap (CI=beta+/-1.5std)

```{r,warning=F,echo=F}
res.mdd3 = res.targetpT.annot %>%
  filter(grepl('MDD3',factor)) %>%
  mutate(mdd3.CI.low = beta-1.5*std,mdd3.CI.high=beta+1.5*std) %>% 
...
res.howard = res.targetpT.annot %>%
  filter(grepl('Howard',factor)) %>% 
  mutate(howard.CI.low = beta-1.5*std,howard.CI.high=beta+1.5*std) %>% 
...
```{r,echo=F,warning=F,fig.height=13,fig.width=10}
figs[[2]]
```
...
```{r,,echo=F,warning=F,fig.height=8,fig.width=13}
figs[[4]]
```

#### Brain and cognition

```{r,,echo=F,warning=F,fig.height=4.5,fig.width=10}
figs[[5]]
```
