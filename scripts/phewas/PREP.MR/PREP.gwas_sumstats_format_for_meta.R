...
reformat_idp <- function(tmp.name,tmp.ref,af.info){
  ls.running = read_tsv('data/MR/running_list_reformat',col_names = F) %>% 
    as.data.frame %>% .$X1 %>% as.vector %>% as.character
...
# Neale lab gwas
reformat_neale <- function(tmp.name,neale.ref){
  ls.running = read_tsv('data/MR/running_list_reformat',col_names = F) %>% 
    as.data.frame %>% .$X1 %>% as.vector %>% as.character
  if(!tmp.name %in% ls.running){
    write_tsv(data.frame(tmp.name),path='data/MR/running_list_reformat',append = T)
    ...
    x = fread(tmp.name)
    x = x %>% 
      left_join(.,neale.ref,by='variant') %>% 
...
ls.f.neale %>% .[! . %in% ls.reformatted] %>% as.list %>% 
  pblapply(reformat_neale,neale.ref=ref.neale) 


# Prepare mtag inputs -----------------------------------------------------

if (!file.exists('data/MR/mtag_list')){
  mtag.input = fields.meta %>% 
    mutate(file_mtag = paste0('data/MR/MR_sumstats/',field_tag)) %>% 
    mutate(file_1=gsub('.txt.gz','_formetal.txt.gz',file_1,fixed = T),
           file_1=gsub('.tsv.gz','_formetal.txt.gz',file_1,fixed = T),
