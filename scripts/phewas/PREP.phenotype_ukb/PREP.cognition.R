...
fields.cog = fields.all[grep('Cognit',fields.all$Path),]

cognition=readRDS('/exports/igmm/eddie/GenScotDepression/shen/bakup.dat/ukb_phenotype/2020-10-imaging-ukb40531/CognitiveFunction.rds')
cognition.2=readRDS('/exports/igmm/eddie/GenScotDepression/shen/bakup.dat/ukb_phenotype/2018-10-phenotypes-ukb24262/CognitiveFunctionOnline.rds')
cognition.2=data.frame(f.eid=cognition.2$f.eid,cognition.2[,!colnames(cognition.2) %in% colnames(cognition)])
...
fields.cog.outcome=read.delim('data/data_dictionary/fields.cog_after_manual_check.txt',header=T,stringsAsFactors=T)

# Process block data (multiple rounds) ----------------------------------------------------------------------------

cog_functions_raw = 
      cognition %>%
      select(matches(paste(paste0('f.',fields.cog.outcome$field_name), collapse="|")))

# find unique list of task * instances
ls.task_instance = strsplit(colnames(cog_functions_raw)[2:ncol(cog_functions_raw)], '\\.')
ls.task_instance = t(bind_cols(ls.task_instance))
...
field.input=data.frame(field.id=fields.cog.outcome$field_name,dat='cog_functions_noblock',stringsAsFactors=F) %>%
            split(., seq(nrow(.)))
...
ls.trailmaking = paste0('f.',fields.cog.outcome$field_name[fields.cog.outcome$Path=='Online follow-up > Cognitive function online > Trail making'])
dat.cog = dat.cog[,!(colnames(dat.cog) %in% ls.trailmaking)]

saveRDS(dat.cog,file='data/cognition.rds')

# Data dictionary ----------------------------------------------------------------------------
fields.cog.new = fields.cog.outcome[fields.cog.outcome$Path!='Online follow-up > Cognitive function online > Trail making',]
fields.cog.new = rbind(fields.cog.new,fields.cog.outcome[grep('20157|20248|20155|20148',fields.cog.outcome$field_name),])

fields.cog.new$category='Cognition'
fields.cog.new$field_tag=fields.cog.new$field_name
fields.cog.new$field_tag[fields.cog.new$field_name %in% c(20157,20248,20155,20148)]=paste0(fields.cog.new$field_tag[fields.cog.new$field_name %in% c(20157,20248,20155,20148)],'.diffs')
fields.cog.new$field_used=fields.cog.new$field_name
fields.cog.new$field_used[fields.cog.new$field_name %in% c(20157,20248,20155,20148)]=c('20157,20156','20248,20247','20155,20149','20148,20147')

write.table(fields.cog.new,file='data/data_dictionary/fields.final.cognition.txt',sep='\t',row.names=F,col.names=T)