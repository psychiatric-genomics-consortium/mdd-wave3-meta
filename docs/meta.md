...
1. Create symlinks to cohorts' summary statistics using harmonised filenames.
...
where `FORMAT_COHORT.POP.hgNN.VERSION` is a key encoding:
...
```
snakemake -j1 resources/sumstats/FORMAT_COHORT.POP.hgNN.VERSION.gz
```
...
that matches the cohort name `COHORT` of the summary stats file (`resources/sumstats/text_COHORT.POP.hgNN.VERSION.gz`). The script WILL be executed as
...
```
snakemake -j1 results/sumstats/daner/daner_COHORT.POP.hgNN.VERSION.gz
```